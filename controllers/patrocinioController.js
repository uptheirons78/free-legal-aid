  ...
  if (req.query.search) {
    ...
    Gratuito.aggregate([
      ...
      if (err) {
        ...
        res.render('gratuito_search', {
          title: 'Lista di Pratiche con Gratuito Patrocinio',
...
exports.non_amesso = async (req, res, next) => {
  try {
    ...
    res.render('gratuito_list', {
      title: 'Lista di Pratiche con Gratuito Patrocinio: NON AMMESSO',
      gratuito_list: list_gratuito
    });
  } catch (err) {
...
exports.gratuito_create_post = [
  // Validiamo i campi.
  body('fascicolo', 'Il fascicolo non deve essere vuoto')
    .isLength({ min: 1 })
    .trim(),
  body('cliente', 'Il cliente non deve essere vuoto')
    .isLength({ min: 1 })
    .trim(),
  body('materia', 'La materia non deve essere vuota')
    .isLength({ min: 1 })
    .trim(),
  body('giudice', 'Il campo giudice non deve essere vuoto')
    .isLength({ min: 1 })
    .trim(),
  body('sede', 'Il campo sede non deve essere vuoto')
    .isLength({ min: 1 })
    .trim(),
  body('data_istanza', 'Data Invalida')
    .optional({ checkFalsy: true })
    .isISO8601(),
  // Sanitizza i campi richiesti - non tutti, soltanto i necessari.
  sanitizeBody('fascicolo')
    .trim()
    .escape(),
  sanitizeBody('cliente')
    .trim()
    .escape(),
  sanitizeBody('materia')
    .trim()
    .escape(),
  sanitizeBody('giudice')
    .trim()
    .escape(),
  sanitizeBody('sede')
    .trim()
  ...
  async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      try {
        ...
        const giudici = await Giudice.find();
        const sedi = await Sede.find();
        res.render('gratuito_form', {
          title: 'Nuova Pratica di Gratuito Patrocinio',
          clienti,
          materie,
          giudici,
          sedi,
  ...
  async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      try {
        ...
        const [clienti, materie, giudici, sedi] = await Promise.All([
          clientiPromise,
          materiaPromise,
          giudiciPromise,
          sediPromise
        ]);
        res.render('gratuito_update', {
