...
exports.clienti_detail = async (req, res) => {
  try {
    ...
    const pratiche_cliente = await Gratuito.find({ cliente: req.params.id })
      .populate('materia')
      .populate('giudice')
      .populate('sede');
    if (cliente == null) {
...
exports.clienti_create_post = [
  body('nome')
    .isLength({ min: 1 })
    .trim()
    .withMessage('Il Nome deve essere specificato!'),
  body('cognome')
    .isLength({ min: 1 })
    .trim()
    .withMessage('Il Cognome deve essere specificato!'),
  body('data_di_nascita', 'Invalid date of birth')
    .optional({ checkFalsy: true })
    .isISO8601(),
  body('luogo_di_nascita')
    .isLength({ min: 1 })
    .trim()
    .withMessage('Il Luogo di Nascita deve essere specificato!'),

  sanitizeBody('nome').trim(),
  sanitizeBody('cognome').trim(),
  sanitizeBody('data_di_nascita').toDate(),
  sanitizeBody('luogo_di_nascita').trim(),
...
exports.clienti_update_post = [
  body('nome', 'Il nome del cliente è necessario!')
    .isLength({ min: 1 })
    .trim(),
  body('cognome', 'Il cognome del cliente è necessario!')
    .isLength({ min: 1 })
    .trim(),
  body('data_di_nascita', 'Data di nascita non valida!')
    .optional({ checkFalsy: true })
    .isISO8601(),
  body('luogo_di_nascita', 'Il luogo di nascita del cliente è necessario!')
    .isLength({ min: 1 })
    .trim(),
